...
앱의 주요 탐색 구조는 "홈", "컬렉션", "예약", "마이"의 4개 하단 탭으로 구성한다.
거래와 이벤트는 보조 화면으로 유지하고, 설정은 마이의 하위 화면으로만 진입한다.
등록/상세/편집 화면은 독립 페이지보다 시트와 다이얼로그 중심으로 구성한다.
Main Screens
Home
- 이번 달 지출 요약
- 보유 굿즈 수량 및 예약 진행 건수
...
My
- 로컬 프로필/동기화 상태
- 보유 수, 활성 예약, 월 지출, 임박 일정 요약
- 거래, 이벤트, 설정으로 이어지는 개인 허브 역할
Secondary Screens
Transactions
- 월별 그룹 리스트
...
Screen Hierarchy
Home Stack
Home > Quick Add Bottom Sheet > Detail Sheet
Collection Stack
Collection List > Collection Detail Sheet > Edit Collection Sheet
Preorder Stack
Preorder List > Preorder Detail Sheet > Edit Preorder Sheet
Transaction Flow
Home or My > Transactions > Transaction Detail Sheet > Edit Transaction Sheet
Event Flow
Home or My > Events > Event Detail Sheet > Edit Event Sheet
My Stack
My > Settings
Cross-Screen Rules
